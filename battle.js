...
function startBattle() {
  ...
  if (battleBuffsString) {
    ...
    if (battleBuffs.damage > 1) {
      addBattleMessage('system', `Quiz buff: Your damage increased by ${Math.floor((battleBuffs.damage - 1) * 100)}%!`);
    } else if (battleBuffs.damage < 1) {
      addBattleMessage('system', `Quiz debuff: Your damage decreased by ${Math.floor((1 - battleBuffs.damage) * 100)}%!`);
    }
