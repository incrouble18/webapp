...
function generateMathProblem() {
    ...
    switch (operation) {
        ...
        case '/':
            ...
            operation = ':'; // Display division as colon
            break;
    }
...
function showProblem() {
    ...
    document.getElementById('problem-number').textContent = `Problem: ${problemNumber} / 20`;
...
function endGame() {
    ...
    // Check for Math Wizard achievement (20 problems in under 1 minute)
    if (correctAnswers >= 20 && elapsedTime < 60) {
    ...
    if (userData.records.mathTime.length > 0) {
        userData.records.mathTime.forEach((record, index) => {
            ...
            recordItem.textContent = `${index + 1}. Time: ${timeStr} - Correct: ${record.correct}/20 - ${record.date}`;
